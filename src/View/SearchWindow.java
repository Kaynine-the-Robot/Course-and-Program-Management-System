...
public class SearchWindow {
	...
	// boolean that determines whether something was found or not, initally false since nothing has been found / searched for
	...
	private void initialize() {
		...
		btnExplore.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				// gotta check if we actually have found something the user searched for
				if(wasFound == true)
				{
					// the selection from the drop down 
					int selection = comboBox.getSelectedIndex();
					if(selection == 0)
					{
						ListWindow.setCurrentFaculty(index);
					}
					else if(selection == 1)
					{
						ListWindow.setCurrentDepartment(index);
					}
					else if(selection == 2)
					{
						ListWindow.setCurrentProgram(index);
					}
					else
					{
						ListWindow.setCurrentCourse(index);
					}
					backGUI.setWindowType(selection);
					System.out.println(selection);
					backGUI.openListWindow(W, H, backGUI.getFlag());
					frmSearch.dispose();
					
				}
				// if we still haven't found anything, i.e. wasFound == false
				else
				{
					WarningMessage nothingHasBeenFound = new WarningMessage("Unable to explore, nothing has been found");
				}
			}
		});
